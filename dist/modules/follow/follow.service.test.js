...
describe("FollowService", () => {
    ...
    beforeEach(() => {
        ...
        follow_repo_1.default.getInstance.mockReturnValue(repoMock);
        users_repo_1.default.getInstance.mockReturnValue(userRepoMock);
        service = follow_service_1.default.getInstance();
    ...
    describe("acceptFollowRequest", () => {
        ...
        it("should accept request and update counters successfully", async () => {
            ...
            repoMock.updateFollowStatus.mockResolvedValue({
                _id: requestId,
                status: follow_types_1.FollowStauts.following,
            });
            ...
            expect(repoMock.updateFollowStatus).toHaveBeenCalledWith(requestId, follow_types_1.FollowStauts.following);
            ...
            expect(result.status).toBe(follow_types_1.FollowStauts.following);
        });
    ...
    describe("getFollowers", () => {
        it("should call repo.getFollowers", async () => {
            ...
            repoMock.getFollowers.mockResolvedValue(mockFollowers);
    ...
    describe("getFollowing", () => {
        it("should call repo.getFollowing", async () => {
            ...
            repoMock.getFollowing.mockResolvedValue(mockFollowing);
    ...
    describe("getRelationshipStatus", () => {
        ...
        it("should return 'following' when follow exists and status is following", async () => {
            const me = "me";
            const target = "target";
            repoMock.getAFollow.mockResolvedValue({ status: follow_types_1.FollowStauts.following });
        ...
        it("should return 'pending' when follow exists and status is pending", async () => {
            const me = "me";
            const target = "target";
            repoMock.getAFollow.mockResolvedValue({ status: follow_types_1.FollowStauts.pending });
            const result = await service.getRelationshipStatus(me, target);
            expect(result).toBe(follow_types_1.FollowStauts.pending);
        });
